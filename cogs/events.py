...
class Events(commands.Cog):
    ...
    async def on_member_join(self, member):
        ...
        embed.add_field(name = 'Hey,', value = '{} you need to see this!'.format(simiid))
    ...
    async def on_member_remove(self, member):
        now = datetime.datetime.now()
        simiid = '<@358992693453652000>'
        uzenet = "{} ({}) is now part of this discord server!".format(member.name, member.id)
        embed = discord.Embed(
            title=f'``{now}:``', 
            description=f'{uzenet}', 
    ...
    async def on_message_delete(self, message):
        ...
        if (message.author.id == 549654750585421825 or uzenet_content.startswith('?')
            ...
            embed = discord.Embed(
                title='``{}:`` '.format(now),
                description='{}'.format(logba_ez_megy),
    ...
    async def on_reaction_add(self, reaction, user):
        ...
        if user.bot:
            ...
            embed = discord.Embed(
                title='``{}:`` '.format(now), 
                description='{}'.format(kiirando_uzenet), 
        ...
        try:
            ...
            if not member.bot:
                ...
                if ((before.channel is None) and (after.channel.guild.id == 399595937409925140) 
                    ...
                    and (member.id != 549654750585421825)):
                        uzenet = '**{}** has just joined **{}**'.format(member.name, after.channel.name)
                        kiirhato = True
                elif ((before.channel.guild.id == 399595937409925140) and (after.channel is None) and 
                    (nem_mute_az_event_oka == True)):
                        uzenet = '**{}** has disconnected from **{}**'.format(member.name, before.channel.name)
                        kiirhato = True
                elif ((before.channel.guild.id != 399595937409925140) and 
                    (after.channel.guild.id == 399595937409925140) and (nem_mute_az_event_oka == True)):
                        uzenet = '**{}** has just joined **{}**'.format(member.name, after.channel.name)
                        kiirhato = True
                ...
                if kiirhato:
                    ...
                    embed.set_thumbnail(url=member.avatar_url)
                    embed.set_author(name=member.name + ' has updated their VoiceState', 
    ...
    async def on_command_error(self, ctx, error):
        ...
        try:
            embed = discord.Embed(
                title='``{}:`` '.format(now),
                description='{}'.format(":interrobang: Command error: {}".format(error)),
