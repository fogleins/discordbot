...
class BalatonSquad(commands.Cog):
    ...
    async def spotifyadd(self, ctx):
        "Ezt a parancsot használva az általad jelenleg hallgatott Spotify számot adhatod az idei siófoki playlisthez. Csak akkor működik, ha a Spotifyod össze van kötve a Discord fiókoddal."
        ...
        if member.activity:
            if member.activity.name == 'Spotify':
                ...
                file = open('/srv/shared/Simi/programozas/discordbot/0zene.txt', 'a')
                file.write('\n')
                # file.write('{}: {} - {},'.format(ctx.author, spotify.artist, spotify.title))
                ...
                embed2.set_author(name = "  {}".format(author), icon_url=member.avatar_url)
                ...
                uzenet = (f"**{ctx.author.name}** has added **{spotify.title}** by **{spotify.artist}**" +
                    " to this year's Siófok playlist")
                embed = discord.Embed(
                    title = f'``{now}:``', 
                ...
                embed.set_author(
                    name=ctx.author.name + ' has added ' + spotify.title + ' to the playlist', 
    ...
    async def zenelink(self, ctx):
        ...
        await ctx.send('A következő linken tudsz számokat adni az idei playlisthez: ' + 
            'https://open.spotify.com/user/simike00/playlist/4Zegv0BzKwKYm8XfUB4ws2?si=iWHsWZ2xRxqeJTjEs8Bdjg')
