...
TOKEN = 'token' # git verzió miatt csak egy sztringet kap, futáshoz meg kell adni a Discord Dev oldalon található tokent
description = 'Szédületes bot in Python'
bot = commands.Bot(command_prefix='?', description=description)
new_Version = False
major_version_change = False
#a main mappáján belül kell egy mappának lennie, ami a cogokat tartalmazza, ezt előtegként hozzá kell illeszteni
extensions = [
    'cogs.adminonly',
    'cogs.events',
    'cogs.commands',
    ...
    if major_version_change == True:
        await bot.get_channel(399595938013773824).send(":loudspeaker: Hey everyone!" +
            "\nI'm making my way out of alpha! This means most of the bugs have been fixed, " +
                "but you still might experience some issues. If you discover any bugs, please " +
...
async def unload(ctx, extension):
    ...
    if ctx.author.id == 358992693453652000:
        try:
            ...
            await ctx.send('Unloaded {} :white_check_mark: '.format(extension), delete_after = 5)
        except Exception as error:
